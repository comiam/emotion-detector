...
def get_new_models():
    ...
    for model_type in allowed_models_to_load:
        ...
        if model_type == 'MLPClassifier':
            ...
            model_params['alpha'] = 10 ** np.random.uniform(-5, -1)
            ...
            model_params['C'] = int(10 ** np.random.uniform(0, 3))
            model_params['solver'] = np.random.choice(['liblinear', 'lbfgs'])
            model_params['max_iter'] = np.random.randint(50, 200)
            model = LogisticRegression(**model_params)

        elif model_type == 'SVC':
            model_params['C'] = 10 ** np.random.uniform(0, 3)
